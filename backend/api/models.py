    ...
    def generate_gltf(self):
        ...
        try:
            ...
            for i, row in df.iterrows():
                ...
                sample_dict = {'name': sample_name, 'colors' : [float(feopigmentos), float(clor_a)], 'variables': [columns[0], columns[1]]}
                samples.append(sample_dict)


            dict['model']["vol"] = volumen
            bpy.ops.mesh.primitive_uv_sphere_add(location=(0, 0, 0), radius = volumen)
            sphere = bpy.context.active_object
            sphere.name = "Volumen"
